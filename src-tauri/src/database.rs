...
"#,
  ...
  if let Some(owner_id) = filters.owner_id.as_ref().filter(|value| !value.trim().is_empty()) {
...
fn build_counts_where_clause(search: Option<String>, owner_id: Option<String>) -> (String, Vec<Box<dyn ToSql>>) {
...
fn entity_sync_status_after_success(
  ...
  Ok(if has_followups > 0 { "pending" } else { "synced" })
}
...
fn mark_inventory_movement_synced_for_item(
  ...
  let payload = serde_json::from_str::<serde_json::Value>(payload_json).unwrap_or(serde_json::Value::Null);
  ...
  if let Some(movement_id) = movement_local_id {
    connection
      ...
      .map_err(|error| format!("No se pudo marcar movimiento local exacto como sincronizado: {error}"))?;
  ...
  let (reason, quantity_change) = match operation_type {
    "adjust" => {
      ...
      (Some(reason.to_string()), Some(if operation == "out" { -quantity } else { quantity }))
    }
    "create_remate" => {
      let stock = payload
        .get("stock")
  ...
  connection
    ...
    .map_err(|error| format!("No se pudo marcar movimiento local por fallback como sincronizado: {error}"))?;
...
pub fn close_local_cash_session(
  ...
  let Some((local_id, remote_id)) = resolve_cash_session_local_id(&transaction, &session_id)? else {
...
pub fn list_local_expenses(
  ...
  let connection = connection_from_state(&state)?;
  let Some((session_local_id, _)) = resolve_cash_session_local_id(&connection, &cash_session_id)? else {
    return Ok(Vec::new());
  };
  ...
  for row in rows {
    ...
    for allocation in allocation_rows {
      allocations.push(
        allocation.map_err(|error| format!("No se pudo mapear asignacion local: {error}"))?,
      );
    }
...
pub fn upsert_local_expense(
  ...
  transaction
    ...
    .map_err(|error| format!("No se pudieron limpiar asignaciones locales del gasto: {error}"))?;
...
pub fn register_local_sale(
  ...
  if let Some(key) = input.idempotency_key.as_ref().filter(|value| !value.trim().is_empty()) {
    let existing = transaction
      ...
      .optional()
      .map_err(|error| format!("No se pudo verificar idempotencia local de venta: {error}"))?;
    ...
    if let Some((sale_id, total)) = existing {
      let item_count = transaction
        ...
        .map_err(|error| format!("No se pudo leer detalle local de venta existente: {error}"))?;
  ...
  for item in &input.items {
    let Some((product_local_id, product_remote_id, product_name, product_barcode, owner_id, current_stock, is_active)) =
      resolve_product_row(&transaction, &item.product_id)?
    else {
      return Err(format!("No se encontro el producto local: {}", item.product_id));
    };
    ...
    transaction
      ...
      .map_err(|error| format!("No se pudo guardar movimiento local de inventario por venta: {error}"))?;
  }
...
pub fn get_local_session_sales_stats(
  ...
  let connection = connection_from_state(&state)?;
  let Some((cash_session_local_id, _)) = resolve_cash_session_local_id(&connection, &cash_session_id)? else {
...
pub fn list_local_sales(
  ...
  for row in rows {
      ...
      .query_map(params![sale_local_id.clone()], |item_row| {
        ...
        Ok(LocalSaleHistoryItem {
          ...
          owner_id: partner
            ...
            .unwrap_or_else(|| partner.as_ref().map(|current| current.id.clone()).unwrap_or_default()),
          partner,
        })
      })
    ...
    for item in item_rows {
      sale_items.push(item.map_err(|error| format!("No se pudo mapear item local de venta: {error}"))?);
    }
...
pub fn get_local_cash_session_report(
  ...
  let rows = statement
    .query_map(params![cash_session_local_id], |row| {
      ...
      Ok(LocalCashSessionReportRow {
        ...
        partner_id: partner_remote_id.unwrap_or_else(|| row.get::<_, String>(3).unwrap_or_default()),
  ...
  for row in rows {
    report.push(row.map_err(|error| format!("No se pudo mapear reporte local de sesion: {error}"))?);
  }
...
pub fn adjust_local_product_stock(
  ...
  let movement_delta = if operation == "in" { quantity } else { -quantity };
...
pub fn list_local_inventory_movements(
  ...
  for row in rows {
    movements.push(row.map_err(|error| format!("No se pudo mapear movimiento local: {error}"))?);
  }
...
pub fn list_local_bodega_products(
  ...
  for row in rows {
    products.push(row.map_err(|error| format!("No se pudo mapear producto de bodega local: {error}"))?);
  }
...
pub fn create_local_remate(
  ...
  transaction
    .execute(
      ...
      params![movement_id, product_local_id.clone(), product_remote_id.clone(), stock],
    )
...
pub fn remove_local_sync_queue_item(
  ...
  let deleted = connection
    .execute("DELETE FROM sync_queue WHERE local_id = ?1", params![item_id])
...
pub fn mark_local_sync_queue_item_synced(
  ...
  let transaction = connection
    .transaction()
    .map_err(|error| format!("No se pudo abrir transaccion local para sync exitoso: {error}"))?;
  ...
  match entity_name.as_str() {
    "cash_sessions" => {
        ...
        .execute(
          "UPDATE cash_sessions
           SET remote_id = COALESCE(remote_id, ?2),
               ...
               updated_at = CURRENT_TIMESTAMP
           WHERE local_id = ?1",
          params![entity_local_id.clone(), effective_remote_id.clone(), entity_sync_status],
        )
        .map_err(|error| format!("No se pudo marcar sesion local como sincronizada: {error}"))?;
      ...
      transaction
        ...
        .map_err(|error| format!("No se pudo propagar remote_id de sesion a ventas locales: {error}"))?;
      ...
      transaction
        ...
        .map_err(|error| format!("No se pudo propagar remote_id de sesion a gastos locales: {error}"))?;
    }
    "sales" => {
      if effective_remote_id.is_none() {
      ...
      transaction
        ...
        .map_err(|error| format!("No se pudo marcar venta local como sincronizada: {error}"))?;
      ...
      transaction
        ...
        .map_err(|error| format!("No se pudo marcar detalle local de venta como sincronizado: {error}"))?;
    ...
    "expenses" => {
      ...
      transaction
        ...
        .map_err(|error| format!("No se pudo marcar gasto local como sincronizado: {error}"))?;
    ...
    "products" => {
      transaction
        ...
        .map_err(|error| format!("No se pudo marcar producto local como sincronizado: {error}"))?;
      ...
      transaction
        ...
        .map_err(|error| format!("No se pudo propagar remote_id del producto a movimientos locales: {error}"))?;
  ...
  transaction
    .execute("DELETE FROM sync_queue WHERE local_id = ?1", params![item_id])
    .map_err(|error| format!("No se pudo remover item sincronizado de la cola local: {error}"))?;
...
#[tauri::command]
pub fn upsert_remote_partners(state: State<'_, DatabaseState>, partners: Vec<LocalPartner>) -> Result<usize, String> {
  let mut connection = connection_from_state(&state)?;
  let transaction = connection
    .transaction()
  ...
  for partner in &partners {
    let local_id = partner.remote_id.clone().unwrap_or_else(|| partner.id.clone());
    transaction
      .execute(
        r#"
...
#[tauri::command]
pub fn list_local_products(state: State<'_, DatabaseState>, filters: ProductQuery) -> Result<Vec<LocalProductWithOwner>, String> {
  let connection = connection_from_state(&state)?;
...
#[tauri::command]
pub fn list_local_product_keys(state: State<'_, DatabaseState>) -> Result<Vec<LocalProductKey>, String> {
  let connection = connection_from_state(&state)?;
  let mut statement = connection
    .prepare(
      "SELECT local_id, remote_id, barcode
       FROM products
       ORDER BY updated_at DESC",
    )
    .map_err(|error| format!("No se pudo preparar lectura local de claves de productos: {error}"))?;
  ...
  let mut products = Vec::new();
  for row in rows {
    products.push(row.map_err(|error| format!("No se pudo mapear clave local de producto: {error}"))?);
  }
...
pub fn count_local_products(
  ...
  let low_sql = format!("SELECT COUNT(*) FROM products{where_clause} AND stock > 0 AND stock <= min_stock");
...
#[tauri::command]
pub fn upsert_remote_products(state: State<'_, DatabaseState>, products: Vec<LocalProduct>) -> Result<usize, String> {
  let mut connection = connection_from_state(&state)?;
  let transaction = connection
    .transaction()
  ...
  for product in &products {
    let local_id = product.remote_id.clone().unwrap_or_else(|| product.id.clone());
...
#[tauri::command]
pub fn find_local_product_by_barcode(state: State<'_, DatabaseState>, barcode: String) -> Result<Option<LocalProductWithOwner>, String> {
  let connection = connection_from_state(&state)?;
  let mut statement = connection
    .prepare(
      r#"
...
pub fn generate_next_local_barcode(state: State<'_, DatabaseState>) -> Result<String, String> {
  ...
  if let Some(value) = last_barcode {
    if let Some(number) = value.strip_prefix("ELB-").and_then(|raw| raw.parse::<i64>().ok()) {
      next_num = number + 1;
    }
  }
...
pub fn upsert_local_product(
  ...
  let remote_id = input
    .remote_id
    .clone()
    .or_else(|| existing.as_ref().and_then(|(_, _, remote_id)| remote_id.clone()));
  ...
  let movement_local_id = if movement_delta != 0 {
    let movement_id = Uuid::new_v4().to_string();
    let reason = if existing.is_some() { "manual_adjustment" } else { "initial_stock" };
...
fn get_default_windows_printer_name() -> Result<String, String> {
  unsafe {
    ...
    if first_call != 0 {
      return Err("Windows devolvio un resultado invalido al consultar la impresora predeterminada.".to_string());
    }
    ...
    let printer_name = String::from_utf16(&buffer[..actual_len])
      .map_err(|error| format!("No se pudo decodificar la impresora predeterminada: {error}"))?;
...
fn print_raw_bytes_to_windows_printer(printer_name: &str, raw_bytes: &[u8]) -> Result<(), String> {
  ...
  unsafe {
    ...
    if OpenPrinterW(printer_name_wide.as_ptr() as *mut u16, &mut printer_handle, null_mut()) == 0 {
      return Err(format!(
...
if ($printers.Count -eq 0) {
  ...
  $printers | ConvertTo-Json -Compress
    ...
    let stdout = String::from_utf8_lossy(&output.stdout).trim().to_string();
    let raw_value: serde_json::Value = serde_json::from_str(if stdout.is_empty() { "[]" } else { &stdout })
    ...
    let raw_printers: Vec<WindowsPrinterInfo> = match raw_value {
      ...
      value @ serde_json::Value::Object(_) => vec![
        serde_json::from_value::<WindowsPrinterInfo>(value)
          .map_err(|error| format!("No se pudo convertir impresora unica: {error}"))?,
      ],
      serde_json::Value::Null => Vec::new(),
      _ => {
        return Err("Windows devolvio un formato inesperado al listar impresoras".to_string());
      }
    };
    ...
    printers.sort_by(|left, right| left.name.to_ascii_lowercase().cmp(&right.name.to_ascii_lowercase()));
    Ok(printers)
  }
}
...
pub fn print_text_ticket_silent(
  ...
  #[cfg(target_os = "windows")]
  {
    ...
    let requested_printer = printer_name
      .unwrap_or_default()
      .trim()
      .to_string();
