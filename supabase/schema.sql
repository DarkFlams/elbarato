...
CREATE INDEX idx_products_stock_revision ON products(stock_revision);
...
-- ============================================
-- ACCESO MOVIL: CODIGOS Y SESIONES
-- ============================================
CREATE TABLE mobile_access_codes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_mobile_access_codes_expires_at ON mobile_access_codes(expires_at);
CREATE INDEX idx_mobile_access_codes_revoked_at ON mobile_access_codes(revoked_at);
...
CREATE TABLE mobile_sessions (
  ...
  revoked_at TIMESTAMPTZ,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT mobile_sessions_scope_check CHECK (scope IN ('stock_only'))
);
...
CREATE TABLE stock_adjustments (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT stock_adjustments_source_check CHECK (source IN ('mobile_count', 'desktop_manual', 'audit_round')),
  CONSTRAINT stock_adjustments_review_status_check CHECK (review_status IN ('auto_applied', 'reviewed_ok', 'reverted'))
);
...
CREATE INDEX idx_stock_adjustments_review_status ON stock_adjustments(review_status);

-- ============================================
-- AUDITORIA POR BLOQUES
-- ============================================
CREATE TABLE stock_audit_rounds (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  partner_id UUID NOT NULL REFERENCES partners(id),
  ...
  CONSTRAINT stock_audit_rounds_status_check CHECK (status IN ('open', 'closed'))
);

CREATE INDEX idx_stock_audit_rounds_partner_id ON stock_audit_rounds(partner_id);
CREATE INDEX idx_stock_audit_rounds_status ON stock_audit_rounds(status);

CREATE TABLE stock_audit_scans (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_stock_audit_scans_round_id ON stock_audit_scans(round_id);
CREATE INDEX idx_stock_audit_scans_product_id ON stock_audit_scans(product_id);
...
CREATE POLICY "auth_all" ON mobile_access_codes FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "auth_all" ON mobile_sessions FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "auth_all" ON stock_adjustments FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "auth_all" ON stock_audit_rounds FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "auth_all" ON stock_audit_scans FOR ALL TO authenticated USING (true) WITH CHECK (true);
